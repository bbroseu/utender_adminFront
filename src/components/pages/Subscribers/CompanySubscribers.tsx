...
export function CompanySubscribers() {
  ...
  // Load company subscribers on component mount
  useEffect(() => {
    loadCompanySubscribers();
  }, []);

  const loadCompanySubscribers = async () => {
    try {
      setLoading(true);
      setApiError(null);
      const data = await sepUsersService.getAll();
      // Filter to show only subscribers with companies
      const subscribersArray = Array.isArray(data) ? data : [];
      const companySubscribers = subscribersArray.filter(
        (subscriber) => subscriber.company && subscriber.company.trim() !== ""
      );
      setSubscribers(companySubscribers);
    } catch (err: any) {
      console.error("Error loading company subscribers:", err);
      setApiError(err.message || "Failed to load company subscribers");
      error(
        "Failed to load company subscribers",
  ...
  const handleExtendClick = (subscriber: SepUser) => {
    ...
    // Set current expiry date as default, or tomorrow if no expiry date
  ...
  const handleExtendConfirm = async () => {
    if (!extendSubscriber || !newExpiryDate) return;
    ...
    try {
      ...
      await sepUsersService.update(extendSubscriber.id, {
        expire_date: Math.floor(selectedDate.getTime() / 1000),
      });
      ...
      success(
        ...
        }" expiry date has been updated to ${selectedDate.toLocaleDateString()}.`
      );
      ...
      <>
        ...
        <div className="flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <Button onClick={loadCompanySubscribers} variant="primary">
              Retry
            </Button>
          </div>
        </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search company subscribers..."
                  value={searchQuery}
              ...
              <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                <div>
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Update the subscription expiration date for{" "}
                        {extendSubscriber.name || extendSubscriber.username}
                      </p>
                    </div>
                    <div className="mt-4">
                      <div className="mb-3 p-3 bg-gray-50 rounded-md text-left">
                        <p className="text-sm text-gray-600">
                          <strong>Current expiry:</strong>{" "}
                          {extendSubscriber.expire_date
                            ? new Date(
                                extendSubscriber.expire_date * 1000
                              ).toLocaleDateString()
                            : "Not set"}
                        </p>
                      </div>
                ...
                <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                  ...
                  >
                    {isExtending ? "Updating..." : "Update Expiration Date"}
                  </Button>
                  <Button
                    type="button"
                    variant="secondary"
