...
export function SendEmail() {
  ...
  // Handle email modal
  const openEmailModal = (user?: SepUser, isAllUsers: boolean = false) => {
    setEmailModal({ isOpen: true, user, isAllUsers });
  ...
  const handleConfirmSend = () => {
    ...
    if (confirmationUser) {
      openEmailModal(confirmationUser);
    } else {
      openEmailModal(undefined, true);
    }
    setConfirmationUser(null);
  };
  ...
  const handleSendEmail = async (e: React.FormEvent) => {
    ...
    if (!emailData.subject.trim() || !emailData.message.trim()) {
      error("Validation Error", "Subject and message are required");
      return;
    }

    try {
      if (emailModal.isAllUsers) {
      ...
      } else if (emailModal.user) {
        // Send to specific user
        await dispatch(
          sendEmailToMember({
            memberId: emailModal.user.id,
            subject: emailData.subject,
            message: emailData.message,
          })
        ).unwrap();
                  ...
                  <div>
                    ...
                    <div className="mt-3 text-center sm:mt-5">
                      <h3 className="text-lg leading-6 font-medium text-gray-900">
                        {emailModal.isAllUsers
                          ? "Send Email to All Active Users"
                          : `Send Email to ${
                              emailModal.user?.name || emailModal.user?.username
                            }`}
                      </h3>
                      ...
                      <p className="text-sm text-gray-500">
                        {confirmationUser ? (
                          ...
                          <>
                            ...
                            {subscribers.length} active users? This action
                            cannot be undone.
                          </>
                        )}
                      </p>
